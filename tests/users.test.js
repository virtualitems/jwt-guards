...
describe('JWT Guards Test Suite', () => {
  ...
  it('1. Login: should return 204 and set access + refresh cookies', async () => {
    ...
    assert.equal(
      response.statusCode,
      204,
      `Login should return 204, got ${response.statusCode}: ${response.body.toString()}`
    )
    assert.ok(response.headers['set-cookie'], 'Should have set-cookie header')
    assert.ok(
  ...
  it('4. Access expired + Refresh expired: should return 401 and clear cookies', async () => {
    ...
    const clearedCookies = setCookies.filter(
  ...
  it('8. Logout: should clear cookies and return 204', async () => {
    ...
    const setCookies = response.headers['set-cookie'] || []
    assert.ok(setCookies.length >= 2, 'Should clear at least 2 cookies')
  ...
  it('10. JWT version change: should return 401 when ver is updated in database', async () => {
    ...
    assert.equal(
      response.statusCode,
      200,
      'Should allow access before version change'
    )
